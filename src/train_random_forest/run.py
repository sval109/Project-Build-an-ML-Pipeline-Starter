...
def go(args):
    ...
    for col in X_val.columns:
        if X_val[col].dtype == 'object':
            # Convert columns with object type to string
            X_val[col] = X_val[col].astype(str)
        elif X_val[col].dtype == 'float64' or X_val[col].dtype == 'int64':
            # Skip these types as they are already consistent
            continue
        else:
            # Handle any other type conversions if needed
